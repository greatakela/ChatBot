...
Для локальной установки проекта нужно склонировать репозиторий ```https://github.com/greatakela/ChatBot.git```, создать среду, затем сделать установку ```pip install -r requirements.txt```. Чат-бот запускается командой ```python app.py```, и открывается в локальном окне браузера на ```http://127.0.0.1:5000```.
...
Контейнер получился "тяжелый" - 13 Гб, несмотря на то, что я исключил из сборки пакеты связанные с GPU, уверен, что проект можно сильно оптимизироваь. Параметры виртуального сервера - 2 x CPU, RAM - 2 Gb, disk space - 80 Gb
