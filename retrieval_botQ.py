...
class ChatBot:
    ...
    def load(self):
        ...
        self.device = torch.device("cpu")    #Q
        self.reranking_model = AutoModelForSequenceClassification.from_pretrained("greatakela/gnlp_hw1_reranker")    #Q
        ...
        self.reranking_model = torch.quantization.quantize_dynamic(
            self.reranking_model, 
            {torch.nn.Linear},  # Quantize only linear layers (best trade-off)
            dtype=torch.qint8,
    ...
    def generate_response(self, utterance: str) -> str:
        ...
        if top_scores[0] < 0.9:
            ...
            #tokenized_utterance = self.tokenizer_reranking(utterance, return_tensors="pt", truncation=True)

            reranked_dict = self.candidates_reranking(
                top_indexes,
            ...
            if reranked_dict:
                updated_top_candidates = dict(sorted(reranked_dict.items(), key=lambda item: item[1]))
